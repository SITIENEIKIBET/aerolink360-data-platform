"""
Spark Structured Streaming consumer for Maintenance.
Reads aaa.maintenance.public.aircraft CDC events, writes clean Bronze Delta.
"""
import logging
...
TOPIC = "aaa.maintenance.public.aircraft"
CHECKPOINT_PATH = "streaming/checkpoints/maintenance_aircraft"
DELTA_OUTPUT_PATH = "streaming/delta/bronze_aircraft"

after_schema = StructType([
    StructField("aircraft_registration", StringType()),
    StructField("model_id", IntegerType()),
    StructField("total_flight_hours", DoubleType()),
    StructField("total_flight_cycles", IntegerType()),
    StructField("is_grounded", BooleanType()),
    StructField("grounded_reason", StringType()),
])
...
def main():
    spark = (
        ...
        .appName("AeroLink360-Maintenance-Bronze-Stream")
    ...
    clean = parsed.filter(F.col("aircraft_registration").isNotNull())
